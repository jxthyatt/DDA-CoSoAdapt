...
from torchvision.transforms import Compose, Resize, RandomCrop, CenterCrop, RandomHorizontalFlip, ToTensor, Normalize, InterpolationMode
import random
import numpy as np
import torch
...
class dataset_multi(data.Dataset):
  def __init__(self, opts):
    ...
    if self.num_domains == 2:
      domains = ['trainA', 'trainB']
    elif self.num_domains > 2:
      domains = ['trainA', 'trainB', 'trainC']
    # also support multi-domain translation
    # domains = ['trainA', 'trainB', 'trainC'...]
    ...
    transforms = [Resize((opts.resize_size, opts.resize_size), InterpolationMode.BICUBIC)]

    # for train_discogan.py
    if opts.phase == 'train':
  ...
  def load_img(self, img_name, input_dim):
    # for eval_discogan.py
    # with open('img_path_ep00399.txt', 'a') as f:
    #   f.write(img_name + '\n')
...
class dataset_single(data.Dataset):
  def __init__(self, opts, domain):
    self.dataroot = opts.dataroot
    domains = ['trainA', 'trainB']
    images = os.listdir(os.path.join(self.dataroot, domains[domain]))
    self.img = [os.path.join(self.dataroot, domains[domain], x) for x in images]
