...
class MD_multi(nn.Module):
    def __init__(self, opts):
        ...
        '''# resume
        self.dis1_opt = torch.optim.Adam([{'params': self.dis1.parameters(), 'initial_lr': lr}], lr=lr, betas=(0.5, 0.999), weight_decay=0.0001)
        self.dis2_opt = torch.optim.Adam([{'params': self.dis2.parameters(), 'initial_lr': lr}], lr=lr, betas=(0.5, 0.999), weight_decay=0.0001)
        self.enc_c_opt = torch.optim.Adam([{'params': self.enc_c.parameters(), 'initial_lr': lr}], lr=lr, betas=(0.5, 0.999), weight_decay=0.0001)
        self.enc_a_opt = torch.optim.Adam([{'params': self.enc_a.parameters(), 'initial_lr': lr}], lr=lr, betas=(0.5, 0.999), weight_decay=0.0001)
        ...
        '''# resume
        self.disContent_opt = torch.optim.Adam([{'params': self.disContent.parameters(), 'initial_lr': lr_dcontent}], lr=lr_dcontent, betas=(0.5, 0.999), weight_decay=0.0001)
        '''
        # train
    ...
    def no_update_D(self, image, c_org):
        self.input = image
        self.c_org = c_org
        self.forward()
        return self.z_content_a, self.z_content_b, self.z_attr_a, self.z_attr_b
    ...
    def backward_G_alone(self):
        ...
        '''
        if self.concat:
            loss_z_L1_a = torch.mean(torch.abs(self.mu2_a - self.z_random)) * 0
            loss_z_L1_b = torch.mean(torch.abs(self.mu2_b - self.z_random)) * 0
        else:
            loss_z_L1_a = torch.mean(torch.abs(self.z_attr_random_a - self.z_random)) * 0
            loss_z_L1_b = torch.mean(torch.abs(self.z_attr_random_b - self.z_random)) * 0
        '''
        if self.concat:
        ...
        loss_z_L1 = loss_z_L1.detach_().requires_grad_(True)
        loss_z_L1.backward()
